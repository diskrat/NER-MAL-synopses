...
- **Extração de texto:** Uso do MinerU, uma ferramenta de código aberto voltada para a extração e conversão de documentos.
...
- **Pipeline principal:** Integração com `spaCy` para extração de entidades
...
- **Entidades extraídas:** `data/processed/entities.jsonl`.
...
- **Achado principal:** 10 documentos são insuficientes para ter uma ideia sobre as temáticas dos TCCs do curso.

### 4.2 Implicações para Engenharia de Computação
- **Documentos analisados:** TCCs e artigos de pesquisa contêm vocabulário técnico (tecnologias, métodos). Hubs emergentes refletem **conceitos-chave** da área.
- **Exemplo:** Em grafos por parágrafo, termos como "Inteligência Artificial", "Python", "Engenharia de Software" emergem como hubs.

### 4.3 Limitações e Próximas Etapas
...
- **Stack:** Python 3.8+, spaCy 3.x, NetworkX, Matplotlib/Seaborn.
- **Códigos-fonte:** 
